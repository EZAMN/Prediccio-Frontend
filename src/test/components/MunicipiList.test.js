...
class MunicipisList extends Component {
  ...
  constructor(props) {
    ...
    this.apiUrl = JSON.parse(config.default.Config).serverUrl + '/municipis/metadades';
    ...
    this.compareMunicipi = this.compareMunicipi.bind(this);
  }
    ...
    return (
      <section className="municipisList">
        ...
        {municipis.map(municipi =>
          <Municipi key={municipi.codi} municipi={municipi} compareMunicipi={(e) => this.compareMunicipi(municipi, e)} removeMunicipi={(e) => this.removeMunicipi(municipi.codi, e)} />
        )}
        </ul>
